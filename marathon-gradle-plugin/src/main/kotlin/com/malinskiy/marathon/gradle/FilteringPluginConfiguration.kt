...
fun Wrapper.toList(): List<TestFilterConfiguration> {
    ...
    this.simpleClassnameFilterFromFile?.map {
        val filterData = it.split(",")
        TestFilterConfiguration.SimpleClassnameFilterFromFile(filterData.first(), filterData[1].toRegex())
    }?.let {
        mutableList.addAll(it)
    }
